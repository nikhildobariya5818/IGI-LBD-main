...
import InvoicePDFGradingScales from "./StandardReports/InvoicePDF-GradingScales"
...
const ReportProcessor = () => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      // If edit mode is enabled, redirect to editor with data
      // Pass the useGradingScales flag to the editor
      if (editReport) {
                ...
                <div className="space-y-3">
                  <div className="flex items-center space-x-2">
                    <input
                      id="grading-scales"
                      type="checkbox"
                      checked={useGradingScales}
                      onChange={(e) => setUseGradingScales(e.target.checked)}
                      className="w-4 h-4 rounded border-gray-300 cursor-pointer"
                    />
                    <Label htmlFor="grading-scales" className="text-sm font-medium cursor-pointer">
                      GRADING SCALES
                    </Label>
                  </div>
                  <div className="flex items-center space-x-2">
                    <input
                      ...
                      type="checkbox"
                      checked={editReport}
                      onChange={(e) => setEditReport(e.target.checked)}
                      className="w-4 h-4 rounded border-gray-300 cursor-pointer"
                    />
                    <Label htmlFor="edit-report" className="text-sm font-medium cursor-pointer">
